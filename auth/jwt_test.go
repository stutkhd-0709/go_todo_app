...
func TestJWTer_GenerateToken(t *testing.T) {
	...
	sut, err := NewJWTer(moq, clock.RealClock{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestJWTer_GetToken(t *testing.T) {
	...
	// reqのインスタンスを作成してるだけなので、本当にリクエストを送信してるわけではない
	req := httptest.NewRequest(http.MethodGet, `https://github.com/stutkhd-0709`, nil)
